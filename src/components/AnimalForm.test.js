...
test("renders Animal Form without errors", () => {
  render(<AnimalForm />);
});
...
test("User can fill out and submit form", async () => {
  ...
  fireEvent.change(speciesInput, {
    target: { value: "canine", name: "species" },
  });
  fireEvent.change(ageInput, { target: { value: "6", name: "age" } });
  fireEvent.change(notesInput, {
    target: { value: "cuteness", name: "notes" },
  });
  ...
  const newAnimalText = await screen.findByText(/canine/i);
